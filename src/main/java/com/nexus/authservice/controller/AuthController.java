...
import com.nexus.authservice.dto.ChangeEmailRequest;
import com.nexus.authservice.dto.ChangePasswordRequest;
...
public class AuthController {
    ...
    @Operation(summary = "Register a new user (USER or ADMIN) -- sends a verification email")
    ...
    @Operation(summary = "Verify email using the token from the registration email")
    ...
    @Operation(summary = "Resend the registration verification email")
    ...
    public ResponseEntity<?> requestEmailChange(
        ...
        try {
            ...
            authService.requestEmailChange(email, request.getCurrentPassword(), request.getNewEmail());

            return ResponseEntity.ok(Map.of(
                "success", true,
                "message", "Verification link sent to your new email address. Your email won't change until you confirm it."
            ));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(Map.of("success", false, "error", e.getMessage()));
    ...
    @Operation(summary = "Confirm an email change using the token sent to the new address")
    ...
    // ==================== PASSWORD RESET ENDPOINTS (forgot password, logged out) ====================
